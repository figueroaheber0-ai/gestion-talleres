        ...
        for (let index = 0; index < this.workshops.length; index += 1) {
            ...
            if (index === 0) {
                await this.prisma.user.create({
                    data: {
                        ...
                        role: 'superadmin',
                        passwordHash: (0, password_util_1.hashPassword)('HerberAdmin2026!'),
                    },
                });
            }
            const mechanics = [];
            for (let employeeIndex = 0; employeeIndex < employeeCount; employeeIndex += 1) {
                const mechanic = await this.prisma.user.create({
                    data: {
                        tenantId: tenant.id,
                        name: this.pick(['Carlos Gomez', 'Miguel Rios', 'Lucia Sosa', 'Nicolas Vega', 'Paula Ibarra', 'Sergio Luna'], random) + ` ${employeeIndex + 1}`,
                        email: `mecanico${employeeIndex + 1}+${workshop.slug}@taller2r.com`,
                        role: 'employee',
                        passwordHash: (0, password_util_1.hashPassword)('1234'),
                    },
                });
            ...
            ].map(([name, sku, stockQuantity, minAlert, price]) => this.prisma.inventoryItem.create({
                data: {
                    tenantId: tenant.id,
                    name: String(name),
                    sku: String(sku),
            ...
            for (let clientIndex = 0; clientIndex < 5; clientIndex += 1) {
                const client = await this.prisma.client.create({
                    data: {
                        tenantId: tenant.id,
                        name: this.pick(['Juan Perez', 'Maria Gonzalez', 'Andrea Lopez', 'Lucas Ferreyra', 'Sofia Ruiz', 'Tomas Acosta'], random) + ` ${clientIndex + 1}`,
            ...
            VALUES (gen_random_uuid()::text, ${client.id}, ${(0, password_util_1.hashPassword)('cliente123')}, NOW(), NOW())
          `;
                }

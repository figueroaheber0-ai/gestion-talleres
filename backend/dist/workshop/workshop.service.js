...
let WorkshopService = class WorkshopService {
    ...
    async getDashboardSummary(user) {
        if (!user.tenantId) {
            throw new common_1.ForbiddenException('Usuario sin taller asociado.');
        }
        const [appointmentsToday, activeOrders, vehiclesInWorkshop, recentAppointments] = await Promise.all([
    ...
    async getVehicleHistory(user, vehicleId) {
        const where = user.audience === 'client'
        ...
        const updates = workOrderIds.length
            ? await this.prisma.$queryRawUnsafe(`SELECT id, "workOrderId" as "workOrderId", title, message, visibility, "createdAt" as "createdAt"
           FROM "ClientUpdate"
    ...
    async getClientPortal(user) {
        ...
        const allVehicles = relatedClients.flatMap((relatedClient) => relatedClient.vehicles.map((vehicle) => ({
            ...vehicle,
            owner: {
                id: relatedClient.id,
        ...
        const workOrderIds = allVehicles.flatMap((vehicle) => vehicle.workOrders.map((order) => order.id));
        const updates = workOrderIds.length
            ? await this.prisma.$queryRawUnsafe(`SELECT id, "workOrderId" as "workOrderId", title, message, visibility, "createdAt" as "createdAt"
           FROM "ClientUpdate"
           WHERE "workOrderId" IN (${this.joinSqlValues(workOrderIds)})
    ...
    async addVehicleToClientPortal(user, input) {
        if (user.audience !== 'client' || !user.clientId) {
            throw new common_1.ForbiddenException('No autorizado.');
        }
        const client = await this.prisma.client.findUnique({
        ...
        if (existingVehicle && existingVehicle.clientId !== client.id) {
            throw new common_1.BadRequestException('Esa patente ya existe en este taller con otro titular. Contacta al taller para validarla.');
        }
        const vehicle = await this.prisma.vehicle.upsert({
            where: {
                tenantId_plate: {
    ...
    async updateClientPortalVehicleProfile(user, vehicleId, input) {
        if (user.audience !== 'client' || !user.clientId) {
            throw new common_1.ForbiddenException('No autorizado.');
        }
    ...
    async getAppointmentsBoard(user) {
        if (!user.tenantId) {
            throw new common_1.ForbiddenException('Usuario sin taller asociado.');
        }
        ...
        const itemsByColumn = {
            ...
            diagnostico: workOrders
                ...
                urgent: /freno|motor|electr|ruido|urgente/i.test(`${order.diagnostic ?? ''} ${order.appointment?.reason ?? ''}`),
    ...
    async moveBoardItem(user, input) {
        if (!user.tenantId) {
            throw new common_1.ForbiddenException('Usuario sin taller asociado.');
        }
        if (input.sourceColumn === input.targetColumn) {
            throw new common_1.BadRequestException('El item ya se encuentra en esa columna.');
        }
        if (input.sourceColumn === 'agendados') {
            ...
            if (appointment.workOrder) {
                throw new common_1.BadRequestException('Este turno ya fue ingresado al taller.');
            }
    ...
    async getWorkOrderDetail(user, workOrderId) {
        if (!user.tenantId) {
            throw new common_1.ForbiddenException('Usuario sin taller asociado.');
        }
        ...
        const updates = await this.prisma.$queryRawUnsafe(`SELECT id, title, message, visibility, "createdAt" as "createdAt"
       FROM "ClientUpdate"
    ...
    async addPartToWorkOrder(user, workOrderId, input) {
        if (!user.tenantId) {
            throw new common_1.ForbiddenException('Usuario sin taller asociado.');
        }
        const order = await this.prisma.workOrder.findFirst({
        ...
        if (!item) {
            throw new common_1.NotFoundException('Repuesto no encontrado.');
        }
    ...
    async updateWorkOrder(user, workOrderId, input) {
        if (!user.tenantId) {
            throw new common_1.ForbiddenException('Usuario sin taller asociado.');
        }
        const order = await this.prisma.workOrder.findFirst({
        ...
        if (input.mechanicId) {
            ...
            if (!mechanic) {
                throw new common_1.BadRequestException('Mecanico no valido para este taller.');
            }
        }
        await this.prisma.workOrder.update({
            ...
            data: {
                ...
                recommendedNextRevisionDate: input.recommendedNextRevisionDate === null
                    ? null
                    : input.recommendedNextRevisionDate
                        ? new Date(input.recommendedNextRevisionDate)
                        : undefined,
                recommendedNextRevisionNote: input.recommendedNextRevisionNote === null
                    ? null
                    : input.recommendedNextRevisionNote,
                mechanicId: input.mechanicId === null ? null : input.mechanicId,
            },
        });
    ...
    async listWorkOrders(user) {
        ...
        return orders.map((order) => {
            const partsInternalCost = order.parts.reduce((total, part) => total + part.internalCost * part.quantity, 0);
            ...
            return {
                ...
                estado: order.status,
                prioridad: margin < 80 ? 'urgente' : order.totalCost >= 300 ? 'alta' : 'normal',
    ...
    async getFinanceSummary(user, period = 'semana') {
        if (!user.tenantId) {
            throw new common_1.ForbiddenException('Usuario sin taller asociado.');
        }
        ...
        orders.forEach((order) => {
            ...
            const costos = order.parts.reduce((total, part) => total + part.internalCost * part.quantity, 0) +
    ...
    async buildClientVehicleWhere(vehicleId, clientId) {
        if (!clientId) {
    ...
    async ensureVehicleProfileTable() {
    ...
    async getVehicleProfiles(vehicleIds) {
        await this.ensureVehicleProfileTable();
    ...
    async resolveRelatedPortalClients(clientId) {
        const client = await this.prisma.client.findUnique({
            where: { id: clientId },
            select: { email: true },
        });
    ...
    groupPortalVehicles(vehicles, updates, profiles) {
        ...
        vehicles.forEach((vehicle) => {
            ...
            existing.workOrders.push(...vehicle.workOrders.map((order) => ({
                ...order,
                tenantId: vehicle.tenantId,
                workshopName: vehicle.tenant.name,
                updates: updates.filter((update) => update.workOrderId === order.id),
        ...
        return Array.from(grouped.values())
            .map((vehicle) => ({
            ...vehicle,
            workshopName: vehicle.workshops.length === 1
                ? vehicle.workshops[0]
                : `${vehicle.workshops[0]} + ${vehicle.workshops.length - 1} talleres`,
            workOrders: vehicle.workOrders.sort((left, right) => new Date(right.createdAt).getTime() - new Date(left.createdAt).getTime()),
        }))
            .sort((left, right) => left.plate.localeCompare(right.plate));
    }
    mapColumnToWorkOrderStatus(column) {
